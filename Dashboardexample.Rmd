...
```{r include=FALSE }
catdata$Date <- lubridate::ymd(catdata$Date)
...
renderPlot({
  ...
  data$Player.Name <- fct_reorder(data$Player.Name, data[,intensity_metric])
  
  ggplot(data, aes_string("Period.Name","Player.Name", fill=intensity_metric)) + 
    geom_tile() +
...
renderPlot({
  ...
  exposure_data() %>%
      ...
      ggplot(aes(week_days, week_date, fill = total)) +
      geom_tile(colour = "white") + 
      facet_wrap(~months) +
      theme_bw() +
      scale_fill_gradient(low = "#39a78e", high = "#a73952", name = workload_metric) + 
      labs(x = "Week of the Month", y = "Weekday") + 
      scale_y_continuous(trans = "reverse", name = "boom")
})
```
***
