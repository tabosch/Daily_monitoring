...
```{r include=FALSE }
catdata$Date <- lubridate::mdy(catdata$Date)
catdata$Week <- lubridate::isoweek(catdata$Date)
catdata$day <- lubridate::wday(catdata$Date, label = TRUE, abbr = FALSE)
...
# Include only session summary, non fitness session data
Catapult_daily <- catdata %>% 
  filter(Period.Number == 0, 
         !Period.Name %in% c("Fitness ", "Fitness PM", "Fitness PM Session", "Fitness"))

Catapult_daily <- Catapult_daily %>% 
  group_by(Player.Name) %>%
  mutate(Meters_scale = scale(Odometer),
         PL_scale = scale(Player.Load)) %>%
  ungroup()
  
levels(Catapult_daily$Period.Name) <- gsub("-4Day","-4 Day", levels(Catapult_daily$Period.Name))

Catapult_daily$Type <- ifelse(Catapult_daily$Period.Name %in% c("-6 Day", 
                                                                "-5 Day", 
                                                                "-4 Day", 
                                                                "-3 Day", 
                                                                "-2 Day", 
                                                                ...
                                                                "-2 Day Regen", 
                                                                "-4 Day PM"), 
                              Catapult_daily$Period.Name, 
                              "Match")

Catapult_daily <- Catapult_daily %>%
  group_by(Player.Name, Type) %>%
  mutate(PLmean = mean(Player.Load, na.rm = T),
         PLsd = sd(Player.Load, na.rm = T)) %>%
  ungroup()

Catapult_daily <- Catapult_daily %>% 
  group_by(Player.Name, Week) %>%
  mutate(Total_Player.Load = sum(Player.Load),
         Total_Distance = sum(Odometer),
         HSR_Distance = sum(Velocity.Band.5.Dist),
         Sprint_Distance = sum(Velocity.Band.6.Dist)) %>%
  ungroup()
...
Catapult_daily_mean <- Catapult_daily %>% 
  group_by(Date, Type, Period.Name, Week) %>% 
  summarize(Player_Load_mean = mean(Player.Load, na.rm=T), 
            Player_Load_max = max(Player.Load, na.rm=T), 
            Total_distance_mean = mean(Odometer, na.rm=T), 
            Total_distance_max = max(Odometer, na.rm=T), 
            Meters_minute_mean = mean(Meters_minute, na.rm=T), 
            Meters_minute_max = max(Meters_minute, na.rm=T), 
            PL_scale = mean(PL_scale, na.rm=T), 
            Meters_scale = mean(Meters_scale, na.rm=T), 
            PLmean = mean(PLmean, na.rm=T), 
            PLsd = mean(PLsd, na.rm=T))

```

```{r load_other_data, include=FALSE, cache=TRUE}
...
renderPlot({
  ...
  start_date <- wellness_date() - days(21)
  last_21_days <- DF_wellness %>% filter(Date > start_date, Date <= wellness_date())
...
renderPlot({
  ...
  VB_D_RPE_2 <- VB_D_RPE %>% filter(Date==wellness_date())
  ...
  ggplot(VB_D_RPE_melt, aes(variable, value, fill=variable)) + 
    geom_bar(stat="identity") + 
    coord_polar(theta = "y", start = 0) + 
    facet_wrap(~Name)+ggtitle(max(VB_D_RPE_2$Date)) + 
    xlab("") + 
    ylab("Max is 5, higher equals easier")
})
```
...
renderPlot({
  ...
  VB4 <- DF_wellness %>% filter(Date==wellness_date())

  VB4$Date <- format(VB4$Date, "%a %b %d")
  
  VB5 <-VB4[,c("Name", "SL_S", "RPE_S", "Sore_S", "Stress_S", "SQ_S")]
  VB5_melt <- melt(VB5, id.vars = "Name")
  ggplot(VB5_melt, aes(reorder(Name, value), value,fill=variable)) + 
    geom_bar(stat="identity") + 
    coord_flip() + 
    xlab("Athlete") + 
    ylab("Scaled Value for Wellness") + 
    ggtitle(max(VB4$Date)) + 
    geom_hline(yintercept = -2.5, colour="red", lty=2) + 
    geom_hline(yintercept = -1, colour="yellow", lty=1, size = 2) + 
    geom_text(aes(Name, value, label=round(value,1)), position=position_stack(vjust=0.5))
})
```
...
<h3>Date</h3>
```{r}
latest_gps_date <- max(catdata$Date)

gps_date_format <- "%b %d %Y"
last_30_gps_dates <- head(unique(format(sort(catdata$Date, decreasing = TRUE), format = gps_date_format)), 30)
selectInput("gps_date", NULL, last_30_gps_dates, selected = format(latest_gps_date, format=gps_date_format))
```

<h3>Workload Metric</h3>
```{r}
intensity_metrics <- c("Player Load" = "Player.Load", 
                       "Total Distance" = "Odometer", 
                       "Meters per Minute" = "Meters_minute")

selectInput("intensity_metric", NULL, intensity_metrics, selected = head(intensity_metrics,1))
```
...
renderPlot({
  ...
  ggplot(data=subset(catdata, Position %in% c("F", "M", "D") & Date == gps_date() & Period.Number > 0), aes(reorder(Period.Name, Period.Number), Player.Load, fill=Meters_minute))+geom_bar(stat="identity")+facet_wrap(~id)+geom_text(aes(y=5,label=Period.Name), angle=90, hjust=-.1, size=3)+theme(axis.title.x=element_blank(),
...
### Drill Intensity
```{r, echo=FALSE, fig.height=1000}
renderPlot({
  library(forcats)
  
  # Dataset is all specific drill data on the GPS Date
  data <- catdata %>% 
    filter(Position %in% c("F", "M", "D"), Date == gps_date(), Period.Number > 0)

  intensity_metric <- input$intensity_metric
  
  data$Period.Name <- fct_reorder(data$Period.Name, data$Period.Number)
  data$Player.Name <- fct_reorder(data$id, data[,intensity_metric])
  
  ggplot(data, aes_string("Period.Name","id", fill=intensity_metric)) + 
    geom_tile() +
    scale_fill_gradient2(low="yellow", mid="green", high="red") +
    theme_minimal() +
...
renderDataTable({
  ...
  datatable(ACWR_Solver,
            ...
            rownames = FALSE) %>% 
    formatStyle('Player.Load', 
                background =  styleColorBar(range(ACWR_Solver$Player.Load) , 'lightblue'),  
                backgroundSize = '95% 50%', 
                backgroundRepeat = 'no-repeat', 
                backgroundPosition = 'center') %>%  
    formatStyle('Odometer', 
                background =  styleColorBar(range(ACWR_Solver$Odometer), 'lightblue'),  
                backgroundSize = '95% 50%', 
                backgroundRepeat = 'no-repeat',
                backgroundPosition = 'center') %>%
    formatStyle('ACR_weighted', 'ACR_weighted', 
                backgroundColor = styleInterval(c(0.8,1.3), c('yellow', 'green', 'red')))
})
...
renderDataTable({
  ...
  SpeedData_table <- SpeedData_New %>% filter(Date >="2017-10-12") %>% select(id, Date, Max.Velocity, DaysSinceMax90, DaysSince90Effort, 
                                                                              DaysSinceMax80, DaysSince80Effort)

  SpeedData_table$Max.Velocity <- round(SpeedData_table$Max.Velocity, 2)

  datatable(SpeedData_table, 
            extensions= c('Scroller', 'FixedColumns'), 
            options = list(scrollY = 400, 
                           paging = TRUE, 
                           scrollX = TRUE, 
                           fixedColumns = list(leftColumns= 4))) %>%  
    formatStyle('id', 'DaysSinceMax90', backgroundColor = styleInterval(c(3,7), c('green', 'yellow', 'red'))) %>%  
    formatStyle('DaysSinceMax90', 'DaysSinceMax90', backgroundColor = styleInterval(c(3,7), c('green', 'yellow', 'red'))) %>%
...
Loading {data-navmenu="Player Report"}
==================================================
...
### Player Select
```{r,echo=FALSE}
athletes <- sort(unique(Catapult_daily$id))
selectInput("Player_report", "Select Athlete", athletes, selected = head(athletes,1))
```
...
barplot2 <- reactive({
  ...
  plot2 <-
    catdata %>%
    filter(id == input$Player_report, 
           Date > max(catdata$Date) - days(21), Period.Number < 1) %>%
...
barplot3 <- reactive({
   plot3 <-
    ...
    filter(id == input$Player_report, 
          Period.Number < 1)
  ggplot(plot3, aes(day, Player.Load))+geom_violin(stat="ydensity")+geom_point(data=subset(plot3, Date >= max(catdata$Date) - lubridate::days(7)),colour="red", size = 3)+geom_text(data=subset(plot3, Date >= max(catdata$Date) - lubridate::days(7)), aes(label=round(Player.Load,0)), colour="red", size = 4, vjust=-.5) +coord_flip()+theme(axis.text=element_text(size=12),axis.title=element_text(size=14,face="bold"),plot.title = element_text(size = 14, face = "bold"))+xlab(" ")+ylab("Player Load (au) ")+ggtitle("Range of PL with Today's PL (Red Dot)")
})
...
Acute Chronic Ratio {data-navmenu="Player Report"}
==================================================
...
```{r,echo=FALSE}
athletes <- sort(unique(Catapult_daily$id))
selectInput("acr_athlete", "Select Athlete", athletes, selected = head(athletes,1))
```

<h3>Workload Metric</h3>
```{r}
workload_metrics <- c("Player Load" = "Player.Load", 
                       "Total Distance" = "Odometer", 
                       "Meters per Minute" = "Meters_minute")

selectInput("acr_workload_metric", NULL, workload_metrics, selected = head(workload_metrics,1))
```

<h3>Chronic Period</h3>
```{r}
chronic_periods <- c("28 Days" = 28, "21 Days" = 21)
selectInput("acr_chronic_period", NULL, chronic_periods, selected = head(chronic_periods,1))
```
...
acr_data <- reactive({
  acute_period <- 7
  chronic_period <- as.numeric(input$acr_chronic_period)
  workload_metric <- input$acr_workload_metric
  athlete_id <- input$acr_athlete
  
  gps_date <- max(Catapult_daily$Date)
  
  start_date <- gps_date - lubridate::days(chronic_period*3)
  
  data <- Catapult_daily %>% 
    filter(id == athlete_id, 
           Date > start_date, Date <= gps_date,
           Period.Number == 0) %>%
    select_at(vars("Date", workload_metric))
  
  all_dates <- data.frame(Date = seq.Date(min(data$Date), max(data$Date), by = "day"))
  
  data <- merge(all_dates, data, all.x = TRUE)
  data[,workload_metric] <- data[,workload_metric] %>% replace_na(0)
  
  data <- data %>% mutate(week = lubridate::week(Date))
  
  data %>%
    group_by(week) %>% 
    summarise_at(vars(workload_metric), funs(acute = sum)) %>%
    mutate(chronic = zoo::rollmean(acute, chronic_period/7, fill = NA, align = "right"),
           acr = (acute / chronic))
  
})
```

### Acute Workload v Acute/Chronic Ratio
``` {r, echo = FALSE} 
renderPlot({
  
  data <- acr_data()
  max_acr <- max(data$acr, na.rm = TRUE)
  max_acute <- max(data$acute, na.rm = TRUE)

  ggplot(data) +
    geom_bar(aes(week, acute), stat = "identity", colour = gray(0.5), fill = gray(0.5), alpha = 0.5) + 
    geom_point(aes(week, acr * (max_acute/max_acr)), size = 3, shape = 21, fill = "white") +
    geom_line(aes(week,acr  * (max_acute/max_acr)), color = "blue") +
    scale_y_continuous(
      name = "Acute Workload",
      sec.axis = sec_axis(~ . * (max_acr/max_acute), name = "A/C Ratio")) +
    scale_x_continuous(breaks = seq(min(data$week), max(data$week))) +
    theme_minimal() +
    theme(
      panel.grid.major = element_blank(), 
      panel.grid.minor = element_blank()
    ) +
    labs(x = "Week #", y = "Acute Workload")
})
```
...
***
Exposure {data-navmenu="Player Report"}
==================================================
...
```{r,echo=FALSE}
athletes <- sort(unique(Catapult_daily$id))
selectInput("exposure_athlete", "Select Athlete", athletes, selected = head(athletes,1))
```
...
selectInput("exposure_workload_metric", NULL, workload_metrics, selected = head(workload_metrics,1))
```
...
exposure_data <- reactive({
  workload_metric <- input$exposure_workload_metric
  athlete_id <- input$exposure_athlete
  
  gps_date <- max(Catapult_daily$Date)
  
  start_date <- gps_date - lubridate::days(180)
  ...
  data %>%
    group_by(Date) %>% 
    summarise_at(vars(workload_metric), funs(workload_metric = sum))
})
```
...
renderPlot({
  ...
  data <- exposure_data() %>%
      mutate(
        week_date = ceiling(day(Date) /7),
        months = lubridate::month(Date, label = TRUE),
        week_days = lubridate::wday(Date, label = TRUE)) %>% 
      group_by(week_date, months, week_days) %>%
      summarise_at(vars(workload_metric), funs(total = sum))
  
  ggplot(data, aes(week_days, week_date, fill = total)) +
      geom_tile(colour = "white") + 
      geom_text(aes(label=workload_metric)) +
      facet_wrap(~months) +
      theme_bw() +
      scale_fill_gradient(low = "#39a78e", high = "#a73952", name = workload_metric) + 
