...
```{r wellness_data, include=FALSE, cache=TRUE}
# Load & prepare Wellness Data
...
```{r prepare_wellness_data, include=FALSE}

wellness_date <- max(DF_wellness$Date)

DF_wellness_daily <- DF_wellness %>% filter(Date >= wellness_date)
...
```{r gps_data, include=FALSE, cache=TRUE}
# Load and prepare GOS data
...
last_30_wellness_dates <- tail(unique(format(sort(DF_wellness$Date), format = wellness_date_format)), 30)
selectInput("wellness_date", NULL, last_30_wellness_dates, selected = format(wellness_date, format=wellness_date_format))
```
...
compliance_data <- reactive({
  wellness_day_data <- DF_wellness %>%
    filter(Date == mdy(input$wellness_date))
...
compliance_guage <- reactive({
  ...
  print(mean(compliance_data()$Screened == TRUE))
...
renderPlot({
  ...
  weekly_data <- subset(DF_wellness, Date >= wellness_date - days(7))
