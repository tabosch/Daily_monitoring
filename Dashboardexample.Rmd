...
```{r, echo=FALSE}
renderDataTable({
  Catapult_sessions <- catdata %>% filter(Period.Number < 1)
  
  Catapult_sessions2 <- Catapult_sessions %>% filter(id %in% c("Player_1", "Player_10", "Player_11", "Player_13", "Player_14", "Player_15", "Player_16", "Player_17", "Player_18", "Player_19", "Player_2", "Player_20", "Player_27", "Player_4", "Player_5", "Player_6", "Player_8", "Player_9")) 
  Catapult_sessions2 <- Catapult_sessions2[order(Catapult_sessions2$id, Catapult_sessions2$Date),] 

Name <- Catapult_sessions2$id

ts <- seq(ymd("2017-05-15"), ymd(Sys.Date()), by="day")
df <- expand.grid(Date = ts, Player.Name = Name)

Test <- full_join(Catapult_sessions2, df, by = c("Date" = "Date" , "id" = "Player.Name")) %>% select(Date, id, Player.Load, Odometer) %>% group_by(id, Date) %>% summarize_all(max)
Test<- Test %>% select(Date, id, Player.Load, Odometer) 
###Make the data set smaller so you only have the variables of interest
Test <- Test %>% filter(Date <= "2017-10-12")
Test[is.na(Test)] <-0
Test <- Test %>% group_by(id) %>% mutate('Chronic' = rollmean(Player.Load, 28, align="right", na.pad=TRUE, na.rm=TRUE))
###Calculate the rolling 28 chronic load
Test <- Test %>% group_by(id) %>% mutate('Acute' = rollmean(Player.Load, 7, align="right", na.pad=TRUE, na.rm=TRUE))
###Calculate the rolling 7 day acute load
Test <- Test %>% group_by(id) %>% mutate(Chronic_weighted = Player.Load *0.07 + (1-0.07) * lag(Chronic),
                                                                     Acute_weighted = Player.Load * 0.25 + (1-0.25)* lag(Acute))
###Calculate the weighted load
Test$ACR_weighted <- Test$Acute_weighted/ Test$Chronic_weighted

x<- 10*(-10:200)
Load_max <- function(x,y,b)  ((x*0.25 + y*0.75) / (x*0.07 + b*0.93)-1.3)
Load_min <- function(x,y,b)  ((x*0.25 + y*0.75) / (x*0.07 + b*0.93)-0.8)

ACWR_Solver <- Test %>% 
  filter(Date >= max(Test$Date)) %>% 
  group_by(id, Date) %>% 
  mutate(Max_Load = min(x[which(Load_max(x,Acute,Chronic) >= 0)]), Min_Load = max(x[which(Load_min(x,Acute,Chronic) <= 0 & Load_min(x,Acute,Chronic) > - 0.1) ]))
...
ACWR_Solver <- round_df(ACWR_Solver, 2)
datatable(ACWR_Solver,extensions= c('Scroller', 'FixedColumns'), options = list(
scrollY = 500, paging = TRUE, scrollX = TRUE, fixedColumns = list(leftColumns= 1)), rownames = FALSE, filter = "top")%>%  formatStyle('Player.Load', background =  styleColorBar(range(ACWR_Solver$Player.Load) , 'lightblue'),  backgroundSize = '95% 50%', backgroundRepeat = 'no-repeat',
backgroundPosition = 'center') %>%  formatStyle('Odometer', background =  styleColorBar(range(ACWR_Solver$Odometer), 'lightblue'),  backgroundSize = '95% 50%', backgroundRepeat = 'no-repeat',
backgroundPosition = 'center') %>%  formatStyle('ACR_weighted', 'ACR_weighted', backgroundColor = styleInterval(c(0.8,1.3), c('yellow', 'green', 'red')))
})
