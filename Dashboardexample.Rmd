...
compliance <- data.frame(Name = unique(DF_wellness$Name)) %>% 
  merge(DF_wellness_daily %>% select(Name, Wellness_Total), all.x = TRUE) %>% 
...
```{r}
renderGauge({
  rate <- compliance %>% summarise(compliance_rate = mean(Screened == TRUE) * 100) %>% .$compliance_rate
...
```{r, echo=FALSE}
renderDataTable(
  datatable(data.frame(names = compliance$Name[compliance$Screened == FALSE]), 
            options = list(dom = 't'), 
            rownames= FALSE, 
            colnames = NULL) %>% 
...
renderPlot({
  ...
  Daily <- ggplot(daily_data, aes(Name, Well_S, fill=Well_S)) +
    ...
    geom_rect(aes(ymin = -Inf, ymax = -2.5, xmin = -Inf, xmax = Inf), fill = "pink", alpha = 0.01) +
    geom_rect(aes(ymin = -2.5, ymax = 0, xmin = -Inf, xmax = Inf), fill = "yellow", alpha = 0.01) +
    geom_rect(aes(ymin = 0, ymax = Inf, xmin = -Inf, xmax = Inf), fill = "green", alpha = 0.01) +
  ...
  weekly_data <- subset(DF_wellness, Date >= max(DF_wellness$Date) - days(7))
  ...
  Weekly <- ggplot(weekly_data, aes(Date, Well_S, colour=Well_S)) +
    ...
    facet_grid(Name ~., switch = "y", scale="free_y") +
...
renderPlot({
ggplot(VB_D_RPE_melt, aes(reorder(Name, value), value, fill=variable))+geom_bar(stat="identity")+scale_y_continuous(breaks = c(0:15), limits = c(0,15), labels= c(0:15))+coord_flip()+ggtitle(max(VB_D_RPE_2$Date))+xlab("")+ylab("Max is 15, higher equals easier")
})
```
...
```{r, echo=FALSE}
renderDataTable(
  datatable(data.frame(names = DF_wellness_daily$Name[DF_wellness_daily$SQ_S < -1.0 ]), 
...
```{r, echo=FALSE}
renderDataTable(
  datatable(data.frame(names = DF_wellness_daily$Name[DF_wellness_daily$SL_S < -1.0 ]), 
            options = list(dom = 't',scrollY = "50px"),
            rownames= FALSE, 
            colnames = NULL) %>% 
    formatStyle("names",backgroundColor="red")
...
```{r, echo=FALSE}
renderDataTable(
  datatable(data.frame(names = DF_wellness_daily$Name[DF_wellness_daily$RPE_S < -1.0 ]), 
options = list(dom = 't',scrollY = "50px"),
            rownames= FALSE, 
            colnames = NULL) %>% 
    formatStyle("names",backgroundColor="red")
...
```{r, echo=FALSE}
renderDataTable(
  datatable(data.frame(names = DF_wellness_daily$Name[DF_wellness_daily$Sore_S < -1.0 ]), 
            options = list(dom = 't',scrollY = "50px"), 
            rownames= FALSE, 
            colnames = NULL) %>% 
    formatStyle("names",backgroundColor="red")
...
```{r, echo=FALSE}
renderDataTable(
  datatable(data.frame(names = DF_wellness_daily$Name[DF_wellness_daily$Stress_S < -1.0 ]), 
            options = list(dom = 't',scrollY = "50px"), 
            rownames= FALSE, 
            colnames = NULL) %>% 
    formatStyle("names",backgroundColor="red")
