...
DF_wellness$day <- lubridate::wday(DF_wellness$Date, label = TRUE, abbr = TRUE)

DF_wellness_daily <- DF_wellness %>% filter(Date >= max(DF_wellness$Date))
...
VB_D_RPE <- DF_wellness[,c("Date","id", "day", "RPE", "Cog_RPE", "B_RPE", "M_RPE")]
...
```{r}
renderGauge({
  rate <- Weigh_in_check %>% summarise(compliance_rate = mean(Weighed_in == "yes") * 100) %>% .$compliance_rate
...
Weekly <- ggplot(data=subset(DF_wellness, Date >= max(DF_wellness$Date) - days(7)), aes(Date, Well_S, colour=Well_S))+
...
DF_wellness_day <- 
  ...
  summarize(Wellness_Total_Scale = mean(Wellness_Total_Scale, na.rm=T))
DF_wellness_day$Wellness_marker_color <- ifelse(DF_wellness_day$Wellness_Total_Scale > 1, "green", ifelse(DF_wellness_day$Wellness_Total_Scale < -1.5, "red", "yellow"))
ggplot(data=subset(DF_wellness, Date >= max(DF_wellness$Date) - days(21)), aes(Date, Wellness_Total_Scale))+geom_bar(data=subset(DF_wellness_day, Date >= max(DF_wellness$Date) - days(21)), aes(fill= Wellness_marker_color), stat="summary", fun.y="mean")+geom_point(aes(shape=Wellness_marker, fill= Wellness_marker_color), stat="identity", size = 3)+scale_shape_identity()+scale_fill_identity()+geom_text(data=subset(DF_wellness, Date >= max(DF_wellness$Date)- days(21)), aes(Date, y= 0, label=day), angle=90, vjust=2)+geom_text_repel(data=subset(DF_wellness, Date >= max(Date)- days(14) & Wellness_Total_Scale < -2.5), aes(Date, Wellness_Total_Scale, label=id))+theme(axis.text.x= element_text(angle=90, size=12))
...
renderPlot({
ggplot(VB_D_RPE_melt, aes(reorder(id, value), value, fill=variable))+geom_bar(stat="identity")+scale_y_continuous(breaks = c(0:15), limits = c(0,15), labels= c(0:15))+coord_flip()+ggtitle(max(VB_D_RPE_2$Date))+xlab("")+ylab("Max is 15, higher equals easier")
})
```
...
ggplot(VB5_melt, aes(reorder(id, value), value, fill=variable))+geom_bar(stat="identity")+coord_flip()+xlab("Athlete")+ylab("Scaled Value for Wellness")+ggtitle(max(VB4$Date))+geom_hline(yintercept = -2.5, colour="red", lty=2)+geom_hline(yintercept = -1, colour="yellow", lty=1, size = 2)+geom_text(aes(id, value, label=round(value,1)), position=position_stack(vjust=0.5))
})
```
...
renderDataTable(
  datatable(data.frame(names = DF_wellness_daily$id[DF_wellness_daily$SQ_S < -1.0 ]), 
            options = list(dom = 't',scrollY = "50px"), 
            rownames= FALSE, 
            colnames = NULL) %>% 
