...
Catapult_daily$Period.Name <- ifelse(Catapult_daily$Period.Name == "-4Day", "-4 Day", Catapult_daily$Period.Name)
...
renderPlot({
  ...
  data <- catdata %>% filter(Position %in% c("F", "M", "D"), Date == gps_date(), Period.Number > 0)
  
  ggplot(data, aes(reorder(Period.Name, Period.Number), reorder(Player.Name, Player.Load), fill=Meters_minute)) + 
    geom_tile() +
